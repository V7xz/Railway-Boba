      ...
      await ch.send({
        embeds: [
          new EmbedBuilder()
            ...
            .setDescription(`Closed due to **${CONFIG.AUTO_CLOSE_HOURS}h** of inactivity.`)
            .setTimestamp()
        ]
      }).catch(() => {});
...
async function handleSlash(interaction) {
  ...
  if (commandName === "dashboard") {
    ...
    chunkArray(all, 6)[0].forEach(([channelId, order]) => {
      e.addFields({
        name:   `${fmt.id(order.orderId)}  ·  ${order.item} (${order.variant})`,
  ...
  if (commandName === "accept") {
    ...
    return interaction.reply({
      embeds: [new EmbedBuilder().setColor(0x57f287).setDescription("✅ Approved and customer notified.")],
      flags: 64
    });
  }
      ...
      await logCh.send({
        embeds: [
          buildOrderEmbed(data)
            .setTitle(`🔔 Payment Submitted — Order ${fmt.id(data.orderId)}`)
            .setDescription(`<@${user.id}> marked their order as paid. Review before approving.`)
        ],
    ...
    await channel.send({
      embeds: [
        new EmbedBuilder()
          ...
          .setDescription("Your payment is under review. An admin will verify it shortly.\nDo **not** click again — it's already submitted.")
          .addFields({ name: "Submitted", value: fmt.ts(Date.now()), inline: true })
      ]
    });
  ...
  if (customId.startsWith("approve_order:")) {
      ...
      await targetCh.send({
        ...
        embeds: [
          new EmbedBuilder()
            .setColor(0x57f287)
            .setTitle("✅ Payment Approved!")
            .setDescription(`Your **${data.item} (${data.variant})** is verified! Product coming shortly. 🎉`)
            .setTimestamp()
        ],
        components: [
          new ActionRowBuilder().addComponents(
    ...
    try {
      await interaction.update({
        embeds: [new EmbedBuilder().setColor(0x57f287).setDescription(`✅ Order ${fmt.id(data.orderId)} approved by <@${user.id}>`)],
        components: []
      });
    } catch {
      ...
      await targetCh.send({
        ...
        embeds: [
          new EmbedBuilder()
            ...
            .setDescription("An admin needs your **payment proof** (screenshot). Please send it here.")
        ]
      });
    }
...
async function handleSelect(interaction) {
  ...
  if (customId === "select_item") {
    ...
    const record = {
      ...
      item: itemName, itemId, emoji: item.emoji,
      variant: variantLabel, variantValue,
      price, paymentMethod: null,
    ...
    await ch.send({
      ...
      components: [
        new ActionRowBuilder().addComponents(
          new ButtonBuilder().setCustomId("choose_payment").setLabel("Choose Payment Method").setStyle(ButtonStyle.Primary).setEmoji("💳")
        ),
        new ActionRowBuilder().addComponents(
          new ButtonBuilder().setCustomId("paid_btn").setLabel("I've Paid ✅").setStyle(ButtonStyle.Success)
        )
      ]
    });
  ...
  if (customId === "select_payment") {
    ...
    if (!data || data.userId !== user.id) {
      for (const [, record] of orderData.entries()) {
        if (record.userId === user.id && record.status === "waiting_payment") {
          data = record;
          break;
        }
      }
    ...
    if (!data || data.userId !== user.id) {
      return safeReply(interaction, { content: "❌ No active order found. Please create an order first." });
    }
...
async function handleModal(interaction) {
  ...
  if (customId.startsWith("modal_reject:")) {
    ...
    return interaction.reply({
      embeds: [new EmbedBuilder().setColor(0xed4245).setDescription(`❌ Order ${fmt.id(data.orderId)} rejected.`)],
      flags: 64
    });
  }
  ...
  if (customId.startsWith("modal_review:")) {
    ...
    return interaction.reply({
      embeds: [new EmbedBuilder().setColor(0x57f287).setDescription(`✅ Thanks for your review! ${starStr}`)],
      flags: 64
    });
  }
}
